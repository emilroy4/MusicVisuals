...
public class emil1 {
    ...
    public emil1(rockstar emil) {
        this.emil = emil; // Constructor that takes a "rockstar" object as a parameter and sets it to the "emil" variable
    }
    ...
    public void render() {
        ...
        for (int i = 0; i < stars.length; i++) {
            ...
            if (stars[i] > emil.width) { // If the star goes off the screen
                stars[i] = emil.random(-emil.width, 0); // Reset its position to a random value on the left side of the screen
            }
        ...
        smallSphereSize = PApplet.map(PApplet.sin(PApplet.radians(smallSphereAngle)), -1, 1, 5, 10); // Map the sine of the angle to a size value for the sphere
